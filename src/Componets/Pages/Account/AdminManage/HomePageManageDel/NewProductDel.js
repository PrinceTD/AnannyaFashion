...
function NewProductDel() {
    const { user } = useAuth()
    const [productLoad, setProductLoad] = useState([]);

    useEffect(() => {
        fetch(`https://powerful-wildwood-87881.herokuapp.com/newin`)
            .then((res) => res.json())
    ...
    const handleDeleteConfirm = (id) => {
        ...
        if (confirm) {
            fetch(
                `https://powerful-wildwood-87881.herokuapp.com/newin/${id}`,
                {
                    method: "DELETE",
                }
            )
